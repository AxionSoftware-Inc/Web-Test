      ...
      <Container className="py-10">
        <GlassCard className="p-6 md:p-8">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {steps.map((step, index) => (
              <div key={step.title} className="rounded-xl bg-white/58 p-4">
                <span className="grid size-8 place-items-center rounded-lg bg-ink text-sm font-semibold text-white">{index + 1}</span>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/practice/algebra/targeted" className="rounded-xl bg-ink px-5 py-3 text-sm font-semibold text-white">
              Practice this skill
            </Link>
