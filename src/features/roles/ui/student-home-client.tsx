    ...
    <div className="grid gap-6">
      <PremiumPanel>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand">Student home</p>
          ...
          <Link href="/student/tests" className="rounded-2xl bg-ink px-5 py-3 text-sm font-semibold text-white">Test ishlash</Link>
        </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <PremiumPanel>
          <div className="flex items-center justify-between gap-3">
            ...
            <Link href="/student/tests" className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-semibold hover:bg-surface-soft">Hammasi</Link>
          </div>
        ...
        <PremiumPanel>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand">Xatolar</p>
          <h2 className="mt-2 text-2xl font-semibold">Nimani o&apos;rganish kerak</h2>
          <div className="mt-5 grid gap-3">
            {weakest.length ? weakest.map((topic) => (
              <Link key={topic.slug} href="/student/mistakes" className="rounded-2xl border border-black/8 bg-white p-4 hover:bg-surface-soft">
                <div className="flex items-center justify-between gap-3">
                  <p className="font-semibold">{topic.topic}</p>
                  <p className="text-sm font-semibold text-black/55">{topic.value}%</p>
                </div>
                <div className="mt-3 h-2 rounded-full bg-[#ecece3]">
                  <div className="h-2 rounded-full bg-brand" style={{ width: `${topic.value}%` }} />
                </div>
              </Link>
      ...
      <PremiumPanel>
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand">Testlar</p>
            <h2 className="mt-2 text-2xl font-semibold">Oxirgi qo&apos;shilgan testlar</h2>
          </div>
          <Link href="/student/tests" className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm font-semibold hover:bg-surface-soft">Katalog</Link>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {latestTests.map((test) => (
            <Link key={test.id} href={`/student/tests/${test.slug}`} className="rounded-2xl border border-black/8 bg-white p-4 hover:bg-surface-soft">
