    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <PremiumPanel>
        ...
        <div className="mt-6 grid gap-4">
          ...
          <button onClick={createTeacher} disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-ink px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"><Plus className="size-4" />Create teacher</button>
      ...
      <PremiumPanel>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {filtered.map((teacher) => (
            <Link key={teacher.id} href={`/schools/teachers/${teacher.id}`} className="rounded-3xl border border-black/8 bg-white p-5 hover:bg-surface-soft">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-xl bg-brand-soft px-3 py-2 text-xs font-semibold text-brand">{teacher.is_active ? "active" : "inactive"}</span>
              </div>
