...
export function PackItemClient({ packSlug, tests }: { packSlug: string; tests: ApiTest[] }) {
  ...
  async function save() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Pack item create failed.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="mx-auto max-w-2xl rounded-[28px] border border-black/8 bg-white/82 p-6 shadow-[0_24px_70px_rgba(0,0,0,0.08)]">
      ...
      <div className="mt-6 grid gap-4">
        ...
        <label className="grid gap-2 text-sm font-semibold text-black/65">
          Order
          <input type="number" value={order} onChange={(event) => setOrder(Number(event.target.value))} className="rounded-2xl border border-black/10 bg-white px-4 py-3" />
        </label>
