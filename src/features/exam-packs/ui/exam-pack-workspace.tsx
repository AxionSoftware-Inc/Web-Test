  ...
  return (
    <QuestPage variant="table">
      <div className="grid gap-5">
        <header className="quest-card overflow-hidden">
          <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="px-5 py-6 sm:px-6">
              ...
              <p className="mt-4 max-w-2xl text-sm leading-6 text-muted">{currentPack.description}</p>
              <div className="mt-6 flex flex-wrap gap-2">
                <Button onClick={copyPackLink} variant="secondary"><Link2 className="size-4" />Copy link</Button>
                <Button onClick={() => download(`${currentPack.slug}-results.csv`, csv, "text/csv;charset=utf-8")} variant="secondary"><Download className="size-4" />Export CSV</Button>
                <Button onClick={() => download(`${currentPack.slug}.json`, JSON.stringify({ version: "1.0", pack: currentPack, tests: exportTests, items, results }, null, 2), "application/json;charset=utf-8")} variant="secondary"><FileJson className="size-4" />Export JSON</Button>
                <Button onClick={() => fileRef.current?.click()} variant="secondary"><Upload className="size-4" />Import CSV</Button>
                <Button onClick={() => jsonRef.current?.click()} variant="secondary"><FileJson className="size-4" />Import JSON</Button>
            ...
            <div className="border-t border-line bg-surface-soft px-5 py-6 sm:px-6 lg:border-l lg:border-t-0">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-subtle">Pack health</p>
        ...
        <section className="mt-4 grid gap-4 px-0 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="quest-card p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Button asChild><Link href={`/exam-packs/${pack.slug}/add-test`}>Advanced add</Link></Button>
            </div>
            <div className="mt-5 grid gap-4 rounded-2xl border border-black/8 bg-surface-soft p-4 lg:grid-cols-[1fr_1fr_90px_auto_auto] lg:items-end">
              ...
              <Button onClick={addItem} disabled={busy || !testId}><Plus className="size-4" />Add</Button>
            </div>
            <div className="mt-5 rounded-2xl border border-black/8 bg-white p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Button onClick={addSelectedTests} disabled={busy || !selectedTestIds.length}>Add {selectedTestIds.length || ""}</Button>
              </div>
          ...
          <aside className="grid gap-4">
            <section className="quest-card p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-4">
                ...
                <Button onClick={savePack} disabled={busy}>Save changes</Button>
              </div>
            </section>
...
function Metric({ icon: Icon, label, value }: { icon: typeof CheckCircle2; label: string; value: string | number }) {
  return <div className="rounded-[var(--radius-card)] border border-line bg-surface p-4"><Icon className="size-4 text-brand" /><p className="mt-3 text-xs font-semibold uppercase tracking-[0.14em] text-subtle">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>;
}
