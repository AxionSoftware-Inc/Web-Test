          ...
          <div className="rounded-lg border border-black/10 bg-white p-5">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {[test.topic_slug, test.subject_slug, test.difficulty, "timed assessment", ...questionTypes].map((item) => (
                <span key={item} className="rounded-md bg-brand-soft px-3 py-2 text-sm font-semibold text-brand">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="rounded-lg border border-black/10 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3">
              {questions.slice(0, 3).map((question, index) => (
                <div key={question.id} className="rounded-md border border-black/10 bg-surface-soft p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-brand">Question {index + 1}</p>
          ...
          <div className="rounded-lg border border-black/10 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {[
                ...
                <div key={title} className="rounded-md border border-black/10 bg-surface-soft p-4">
