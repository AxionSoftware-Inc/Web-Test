      ...
      <Container className="py-10">
        <section className="grid gap-5 lg:grid-cols-[1fr_340px]">
          <GlassCard className="p-6">
            <p className="text-sm font-semibold text-brand">{question.topic} / {question.difficulty}</p>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {question.skill_titles.map((skill) => (
                <span key={skill} className="rounded-xl bg-brand-soft px-3 py-2 text-sm font-semibold text-brand">{skill}</span>
              ))}
            </div>
          </GlassCard>
          <GlassCard className="p-5">
            ...
            <div className="mt-4 grid gap-3">
              <Link href={`/questions/${question.id}/solve`} className="rounded-xl bg-ink px-4 py-3 text-center text-sm font-semibold text-white">Solve</Link>
